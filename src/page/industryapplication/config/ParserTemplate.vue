<template>
	<div class="container">
		...
		<el-button
			...
			v-has3="'/industrymanage/tongtianManage/parserTemplate/add'"
		>
			添加
		</el-button>
		...
		>
			...
			<el-table-column prop="parserTemplateId" label="模板ID" sortable></el-table-column>
			<el-table-column prop="channelSystemId" label="渠道系统ID" sortable></el-table-column>
			<el-table-column prop="outTransCodeId" label="外部交易编号"></el-table-column>
			<el-table-column
				prop="gatewayMode"
				label="supergw类型(客户端/服务端)"
				sortable
			></el-table-column>
			<el-table-column prop="groovyContent" label="Groovy模板内容" sortable></el-table-column>

			<el-table-column prop="groovyClassName" label="Groovy脚本类名" sortable></el-table-column>
			...
			<el-table-column fixed="right" label="操作" width="170">
				<template slot-scope="scope">
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/parserTemplate/publish'"
					></el-button>
					<el-button
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/parserTemplate/update'"
					></el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		>
			<div>
				<el-form
					...
					class="demo-ruleForm"
				>
					<el-form-item label-width="30%" label="模板ID" prop="parserTemplateId">
						<el-input
							type="text"
							v-model="uriRuleAddContent.parserTemplateId"
							auto-complete="off"
						></el-input>
					</el-form-item>
					...
					<el-form-item label-width="30%" label="supergw类型" prop="gatewayMode">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleAddContent.gatewayMode"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="Groovy模板内容" prop="groovyContent">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleAddContent.groovyContent"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="Groovy脚本类名" prop="groovyClassName">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleAddContent.groovyClassName"
							auto-complete="off"
						></el-input>
					</el-form-item>
			...
			<div>
				...
				>
					<el-form-item label-width="30%" label="模板ID" prop="parserTemplateId">
						<el-input
							type="text"
							v-model="uriRuleUpdateContent.parserTemplateId"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="渠道系统Id" prop="channelSystemId">
					...
					<el-form-item label-width="30%" label="supergw类型" prop="gatewayMode">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleUpdateContent.gatewayMode"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="Groovy模板内容" prop="groovyContent">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleUpdateContent.groovyContent"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="Groovy脚本类名" prop="groovyClassName">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleUpdateContent.groovyClassName"
							auto-complete="off"
						></el-input>
					</el-form-item>
					<el-form-item label-width="30%" label="备注">
...
export default {
	data() {
		return {
			...
			// 校验规则
			rules: {
				parserTemplateId: [{ required: true, message: '请输入模板ID', trigger: 'blur' }],
	...
	methods: {
		...
		loadData: function () {
			...
			this.searchList.forEach((it) => {
				switch (it.searchType) {
					case 'parserTemplateId':
						parserTemplateId = it.searchInput;
						break;
			...
			// 获取银行渠道系统表数据
			this.$http
				.post('/api/industrymanage/tongtianManage/parserTemplate/list', {
		...
		addUriRule() {
			this.$refs.uriRuleAddContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/parserTemplate/add', {
							parserTemplateId: this.uriRuleAddContent.parserTemplateId,
							channelSystemId: this.uriRuleAddContent.channelSystemId,
							outTransCodeId: this.uriRuleAddContent.outTransCodeId,
							gatewayMode: this.uriRuleAddContent.gatewayMode,
							groovyContent: this.uriRuleAddContent.groovyContent,
							groovyClassName: this.uriRuleAddContent.groovyClassName,
		...
		updateUriRule() {
			this.$refs.uriRuleUpdateContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/parserTemplate/update', {
							id: this.uriRuleUpdateContent.id,
							parserTemplateId: this.uriRuleUpdateContent.parserTemplateId,
							channelSystemId: this.uriRuleUpdateContent.channelSystemId,
							outTransCodeId: this.uriRuleUpdateContent.outTransCodeId,
							gatewayMode: this.uriRuleUpdateContent.gatewayMode,
							groovyContent: this.uriRuleUpdateContent.groovyContent,
							groovyClassName: this.uriRuleUpdateContent.groovyClassName,
		...
		updateStatus(row) {
			...
			this.$confirm('此操作影响较大, 是否继续?', '提示', {
				...
				this.$http
					.get('/api/industrymanage/tongtianManage/parserTemplate/publish', {
						params: {
							groovyClassName: row.groovyClassName,
							status: status,
							modifier: this.$cookie.get('nickname'),
						},
					})
