<template>
	<div class="container">
		...
		<el-button
			...
			v-has3="'/industrymanage/tongtianManage/transTypeChannelMap/add'"
		>
			添加
		</el-button>
		...
		>
			...
			<el-table-column prop="channelSystemId" label="渠道系统ID"></el-table-column>
			...
			<el-table-column fixed="right" label="操作" width="170">
				<template slot-scope="scope">
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/transTypeChannelMap/publish'"
					></el-button>
					<el-button
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/transTypeChannelMap/update'"
					></el-button>
				</template>
			</el-table-column>
		</el-table>
...
export default {
	data() {
		return {
			...
			// 校验规则
			rules: {
				transTypeId: [{ required: true, message: '请输入交易类型ID', trigger: 'blur' }],
	...
	methods: {
		...
		loadData: function () {
			...
			// 获取银行渠道系统表数据
			this.$http
				.post('/api/industrymanage/tongtianManage/transTypeChannelMap/list', {
					transTypeId: transTypeId,
					transTypeName: channelSystemId,
		...
		addUriRule() {
			this.$refs.uriRuleAddContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/transTypeChannelMap/add', {
		...
		updateUriRule() {
			this.$refs.uriRuleUpdateContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/transTypeChannelMap/update', {
		...
		updateStatus(row) {
			...
			this.$confirm('此操作影响较大, 是否继续?', '提示', {
				...
				this.$http
					.get('/api/industrymanage/tongtianManage/transTypeChannelMap/publish', {
						params: {
