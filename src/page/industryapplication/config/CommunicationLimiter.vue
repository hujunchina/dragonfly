<template>
	<div class="container">
		...
		<el-button
			...
			v-has3="'/industrymanage/tongtianManage/communicationLimiter/add'"
		>
			添加
		</el-button>
		...
		>
			...
			<el-table-column prop="maxCounter" label="允许最大的并发数" sortable></el-table-column>
			...
			<el-table-column fixed="right" label="操作" width="170">
				<template slot-scope="scope">
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/communicationLimiter/publish'"
					></el-button>
					<el-button
					...
					<el-button
						...
						circle
						size="small"
						v-has3="'/industrymanage/tongtianManage/communicationLimiter/update'"
					></el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		>
			<div>
				...
				>
					...
					<el-form-item label-width="30%" label="允许最大并发数量" prop="maxCounter">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleAddContent.maxCounter"
							auto-complete="off"
						></el-input>
					</el-form-item>
				</el-form>
				...
				>
					...
					<el-form-item label-width="30%" label="允许最大并发数量" prop="maxCounter">
						<el-input
							autosize
							type="textarea"
							v-model="uriRuleUpdateContent.maxCounter"
							auto-complete="off"
						></el-input>
					</el-form-item>
				</el-form>
...
export default {
	data() {
		return {
			...
			rules: {
				...
				maxCounter: [{ required: true, message: '请输入允许的最大并发数量', trigger: 'blur' }],
			},
	...
	methods: {
		...
		loadData: function () {
			...
			// 获取银行渠道系统表数据
			this.$http
				.post('/api/industrymanage/tongtianManage/communicationLimiter/list', {
		...
		addUriRule() {
			this.$refs.uriRuleAddContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/communicationLimiter/add', {
							communicationId: this.uriRuleAddContent.communicationId,
							maxCounter: this.uriRuleAddContent.maxCounter,
							status: 0,
		...
		updateUriRule() {
			this.$refs.uriRuleUpdateContent.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/industrymanage/tongtianManage/communicationLimiter/update', {
							id: this.uriRuleUpdateContent.id,
							communicationId: this.uriRuleUpdateContent.communicationId,
							maxCounter: this.uriRuleUpdateContent.maxCounter,
							modifier: this.$cookie.get('nickname'),
						})
						.then((res) => {
							if (res.data.code == 100) {
		...
		updateStatus(row) {
			...
			this.$confirm('此操作影响较大, 是否继续?', '提示', {
				...
				this.$http
					.get('/api/industrymanage/tongtianManage/communicationLimiter/publish', {
						params: {
