...
export default {
	...
	methods: {
		...
		loadData: function () {
			...
			if (moduleId) {
				this.$http.get('/api/hotel/certainModule/permissions?moduleId=' + moduleId).then((res) => {
			...
			// 加载所有权限点列表
			this.$http.get('/api/hotel/permissions').then((res) => {
				if (res.data.code == 100) {
		...
		savePermissionModule: function () {
			this.$refs.permissionModule.validate((valid) => {
				if (valid) {
					this.$http
						.post('/api/hotel/module/permission', {
		...
		returnPermissionModuleManage: function () {
			this.$router.push({ name: 'hotelPermissionManage' });
		},
	},
};
</script>
