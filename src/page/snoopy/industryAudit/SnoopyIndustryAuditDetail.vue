...
<template>
	<div class="container">
		...
		>
			...
			<el-form-item label="备注:" v-if="industryInfo.remark !== ''">
				<span>{{ industryInfo.remark }}</span>
			</el-form-item>
			...
			<el-form-item label="审核备注:" v-if="industryInfo.auditStatus !== 0">
				<span>{{ industryInfo.auditRemark }}</span>
			</el-form-item>
			<el-form-item>
				<el-button
					type="success"
				...
				<el-button @click="goSnoopyIndustryAudit()">返回</el-button>
			</el-form-item>
		</el-form>
		<!-- 审核通过确认框 -->
...
export default {
	...
	methods: {
		...
		loadData: function () {
			...
			this.$http.get('/api/snoopy/industry/audit/record?id=' + id).then((res) => {
		...
		commitAuditIndustryInfo: function (id, auditStatus) {
			this.$http
				.post('/api/snoopy/industry/audit', {
					id: id,
		...
		// 跳转行业审核列表界面
		goSnoopyIndustryAudit: function () {
			this.$router.push({ name: 'snoopyIndustryAudit' });
		},
	},
};
</script>
