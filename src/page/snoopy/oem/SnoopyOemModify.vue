	...
	<div class="container">
		...
		>
			<el-form-item label="企业名称:" prop="enterpriseName">
				<el-input class="inputText" v-model="oemInfo.enterpriseName" :disabled="true"></el-input>
			</el-form-item>
			<el-form-item label="认证状态:" prop="authStatus">
				<el-input class="inputText" v-model="oemInfo.authStatus" :disabled="true"></el-input>
			</el-form-item>
			<el-form-item label="行业域名:" prop="domainName">
				<el-input
					class="inputText"
					placeholder="请输入行业域名"
					v-model="oemInfo.domainName"
					clearable
				></el-input>
			</el-form-item>
			...
			<el-form-item label="Logo:">
				<el-upload
					...
					action="/api/snoopy/fileUpload"
					:show-file-list="false"
					:on-success="handleLogoAvatarSuccess"
					:data="paramsData"
					:before-upload="beforeAvatarUpload"
				>
					<img v-if="oemInfo.logoUrl" :src="oemInfo.logoUrl" class="avatar" />
			...
			<el-form-item label="icon:">
				<el-upload
					...
					:before-upload="beforeAvatarUpload"
				>
					<img v-if="oemInfo.iconUrl" :src="oemInfo.iconUrl" class="avatar" />
					<i v-else class="el-icon-plus avatar-uploader-icon"></i>
				</el-upload>
			</el-form-item>
			...
			<el-form-item>
				<el-button type="success" @click="saveOemInfo()" :loading="commitLoading">保 存</el-button>
				<el-button style="margin-left: 20px;" @click="returnOemManage()">返 回</el-button>
				<el-link
					type="primary"
					style="margin-left: 20px;"
					target="_blank"
					:href="'https://' + oemInfo.domainName + '/userAgreement?type=1'"
				>
					预览服务条款
				</el-link>
				<el-link
					...
					style="margin-left: 20px;"
					target="_blank"
					:href="'https://' + oemInfo.domainName + '/userAgreement?type=2'"
				>
					预览法律声明
				</el-link>
				<el-link
					...
					style="margin-left: 20px;"
					target="_blank"
					:href="'https://' + oemInfo.domainName + '/userAgreement?type=3'"
				>
					预览隐私政策
				</el-link>
			</el-form-item>
...
export default {
	...
	methods: {
		handleLogoAvatarSuccess(res, file) {
			this.oemInfo.logoUrl = URL.createObjectURL(file.raw);
			this.logoTmpFileId = res.result.result.tmp_file_id;
		},
		handleIconAvatarSuccess(res, file) {
			this.oemInfo.iconUrl = URL.createObjectURL(file.raw);
		...
		loadData: function () {
			...
			if (uid) {
				this.$http.get('/api/snoopy/oem/oem-info?uid=' + uid).then((res) => {
					if (res.data.code == 100) {
						this.oemInfo = res.data.result;
						if (this.oemInfo.authStatus == 90 || this.oemInfo.authStatus == 100) {
							this.oemInfo.authStatus = '认证完成';
						}
					} else {
		...
		// 保存品牌商OEM信息
		saveOemInfo: function () {
			this.$refs.oemInfo.validate((valid) => {
				if (valid) {
					...
					this.$http
						.post('/api/snoopy/oem/oem-info', {
							uid: this.oemInfo.uid,
							domainName: this.oemInfo.domainName,
							title: this.oemInfo.title,
							englishTitle: this.oemInfo.englishTitle,
		...
		// 返回OEM列表界面
		returnOemManage: function () {
			this.$router.push({ name: 'snoopyOemManage' });
		},
	},
};
</script>
